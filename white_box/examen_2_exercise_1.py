# -*- coding: utf-8 -*-
"""User registration and login system using salted password hashing."""
...
def generate_password_hash(password, salt):
    """Generates a SHA-256 hash from the password and salt."""
    return hashlib.sha256(password.encode() + salt.encode()).hexdigest()


def load_user_data():
    """Loads user data from the JSON file, or returns an empty dict if it doesn't exist."""
...
def login(username, password):
    """Logs in a user by validating their password."""
    user_data = load_user_data()
...
def main():
    """CLI for registering and logging in users."""
    while True:
        print("\n1. Register\n2. Login\n3. Exit")
